...
async function buildZoneStats(rows: RawRow[], weekMonday: string): Promise<ZoneWeeklyStats[]> {
	...
	// Group prior rows by zone (skip nulls)
...
export async function generateWeeklyReport(weekOf?: string): Promise<WeeklyReport> {
	...
	const scoredRows: RawRow[] = rawRows.filter(
		(r): r is RawRow =>
			r.overallScore !== null &&
			r.clarityScore !== null &&
			r.actionabilityScore !== null &&
			r.decisionConfidence !== null,
	);

	const [byForecaster, byZone] = await Promise.all([
		Promise.resolve(buildForecasterStats(scoredRows)),
		buildZoneStats(scoredRows, weekMonday),
	]);
	...
	const overallAvgScore = avg(scoredRows.map((r) => r.overallScore));
	const totalForecastsScored = new Set(scoredRows.map((r) => r.forecastId)).size;
